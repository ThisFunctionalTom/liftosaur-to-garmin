...
test('manifest and icons work under a project path; cached app converts offline', async ({ page, context }) => {
  await openApp(page);
  ...
  await page.route('https://www.liftosaur.com/api/v1/history*', route => route.fulfill({ json: { records: [], hasMore: false } }));
...
test('an update waits for consent, then reloads and cleans only this app’s old cache', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: 'Try an example' }).click();
  const input = await page.getByLabel('Liftosaur API workout text').inputValue();
  ...
  await expect(page.getByLabel('Liftosaur API workout text')).toHaveValue(input);
  await expect(page.locator('#app-status')).toContainText('Download any prepared files');
  await Promise.all([
    page.waitForEvent('load'),
    page.getByRole('button', { name: 'Update and reload' }).click(),
  ]);
  await expect(page.getByLabel('Liftosaur API workout text')).toHaveValue('');
